...
function ozfcCalculate(it,r,fx=0){
 ...
 const sum=dims.reduce((a,b)=>a+b),useVolume=r.divisor>0&&sum>Number(r.volumetric_min_sum||0),volume=useVolume?dims.reduce((a,b)=>a*b)*quantity/r.divisor:0,raw=Math.max(kg,volume),bill=r.step>0?Math.ceil(raw/r.step-1e-10)*r.step:raw;
 ...
 if(!r.no_value_limit&&(limitValue<r.min_value||(r.value_exclusive&&limitValue<=r.min_value)||limitValue>r.max_value))reasons.push(`货值需 ${r.value_exclusive?'＞':'≥'}${r.min_value} 且 ≤${r.max_value} ${limitCurrency}`);
 ...
 if(r.min_sorted_sides?.length&&dims.slice().sort((a,b)=>b-a).some((v,i)=>v<r.min_sorted_sides[i]))reasons.push(`包装尺寸至少需 ${r.min_sorted_sides.join(' × ')} cm（按长边排序）`);
 ...
 steps.push(useVolume?`③ 体积重：${dims.join(' × ')} × ${quantity}件 ÷ ${r.divisor} = ${volume.toFixed(4)} kg；与实重取较大值`:`③ 按实际重量计费，不计体积重${r.volumetric_min_sum?'（三边和未超过 '+r.volumetric_min_sum+' cm）':''}`);
...
class OzFreightCanvas{
 ...
 suggestedRouteState(n,parcel,route,price){
  if(!Number.isFinite(price)||price<=0)return {kind:'pending',label:'待计算建议售价',detail:'先取得有效的成本、运费和佣金，才能校验建议售价是否符合此渠道限制。'};
  const total=Number(n.item.quantity)||1,split=n.packing?.mode&&n.packing.mode!=='none',row=parcel.row;
  const base={...this.shippingItem(n),value:price/total,value_currency:'CNY'};
  const item=split?{...base,length:row.length,width:row.width,height:row.height,weight:row.weight,quantity:1,value:price*Number(row.quantity)/total}:base;
  const result=ozfcCalculate(item,route,this.c.cny_per_rub);
  return {kind:result.eligible?'ready':'blocked',label:result.eligible?'建议售价下仍可用':'建议售价下不可用',detail:'整单建议售价 ¥'+price.toFixed(2)+(split?' · 本包分摊货值 ¥'+item.value.toFixed(2):'')+'；'+(result.eligible?'按当前包装及渠道限制校验通过。仅为运价表规则校验，不代表平台承诺。':result.reasons.join('；'))};
 }
 suggestedRouteHTML(n,parcel,route,price){const state=this.suggestedRouteState(n,parcel,route,price);return '<div class="fc-suggested-route '+state.kind+'" data-suggested-route="'+this.e(route.id)+'" data-suggested-parcel="'+parcel.index+'" title="'+this.e(state.detail)+'"><span>'+({ready:'✓',blocked:'!',pending:'·'}[state.kind])+'</span> '+this.e(state.label)+'</div>';}
 updateSuggestedRouteBadges(n){
  const price=this.estimate(n).x.price,parcels=this.parcelResults(n),routes=new Map(this.c.routes.map(r=>[r.id,r]));
  this.root.querySelectorAll('.fc-drag[data-n="'+n.id+'"][data-q]').forEach(header=>header.closest('.fc-card').querySelectorAll('[data-suggested-route]').forEach(el=>{
   const parcel=parcels[Number(el.dataset.suggestedParcel)],route=routes.get(el.dataset.suggestedRoute);if(!parcel||!route)return;
   const state=this.suggestedRouteState(n,parcel,route,price);el.className='fc-suggested-route '+state.kind;el.title=state.detail;el.innerHTML='<span>'+({ready:'✓',blocked:'!',pending:'·'}[state.kind])+'</span> '+this.e(state.label);
  }));
 }
 ...
 prepare(s,c){
  let converted=false;
  if(c.xy_extra_snapshot!==1&&Array.isArray(c.routes)&&this.bootData?.defaults?.routes){
   for(const r of this.bootData.defaults.routes.filter(r=>r.id.startsWith('兴远-XY-')))if(!c.routes.some(x=>x.id===r.id)){c.routes.push(structuredClone(r));converted=true;}
   c.xy_extra_snapshot=1;converted=true;
  }
  if(c.cel_rfbs_snapshot!==1&&Array.isArray(c.routes)&&this.bootData?.defaults?.routes){
   for(const r of this.bootData.defaults.routes.filter(r=>r.id.startsWith('CEL-RFBS-')))if(!c.routes.some(x=>x.id===r.id)){c.routes.push(structuredClone(r));converted=true;}
   c.cel_rfbs_snapshot=1;converted=true;
  }
...
const versions='<label class="fc-rival-version-label">报价版本<select data-rival-version="'+n.id+'" data-rival-key="'+this.e(current.key)+'"><option value="current" '+(p===current?'selected':'')+'>最新 · 第'+(current.version||1)+'版</option>'+(current.history||[]).slice().sort((a,b)=>b.version-a.version).map(v=>'<option value="'+v.version+'" '+(p===v?'selected':'')+'>历史 · 第'+v.version+'版 · '+this.e(date(v.collectedAt||v.importedAt))+'</option>').join('')+'</select></label>'+(current.historyMore?'<button class="fc-history-more" data-rival-more="'+n.id+'" data-rival-key="'+this.e(current.key)+'">加载更多历史版本</button>':'');
 ...
 updateMaterialCalculation(id,shipping=false){
  ...
  this.updateSuggestedRouteBadges(n);
  this.topCommission();
...
if(!automatic)try{await this.hydrateHistory();}catch(e){this.status('数据已保存，报价显示加载失败，请重新打开画布');}
