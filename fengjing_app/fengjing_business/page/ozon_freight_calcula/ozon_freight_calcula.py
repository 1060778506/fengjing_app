...
def _seller_read(row, path, payload):
    ...
    if path not in ("/v3/product/info/list", "/v5/product/info/prices"):
...
def item_details(item_code, force=0):
    ...
    for b in ozon_bindings:
        ...
        if cached and "commissions" not in cached: cached = None
        try:
...
def save_canvas(title, canvas, config, name=None, modified=None):
    ...
    if not isinstance(view, dict) or any(not isinstance(view.get(k), (int, float)) or not math.isfinite(view[k]) for k in ("x", "y", "z")) or not .25 <= view["z"] <= 2:
    ...
    for node in canvas["nodes"]:
        ...
        ids.add(identifier)
        positions = [node] + [node[k] for k in ("quote", "cost", "sale") if k in node]
